...
public class PlayerData
{
    ...
    private PlayerData(final MMOPlayerData mmoData) {
    ...
    public void updateInventory() {
        ...
        final ItemStack itemInMainHand = this.getPlayer().getInventory().getItemInMainHand();
        if (itemInMainHand.getType().isItem()) {
            ...
            if (nbtItem != null) {
                ...
                if (value != null) {
                    equipmentType = value.getEquipmentType();
                }
            }
        }
        for (final EquippedItem equippedItem : MMOItems.plugin.getInventory().getInventory(this.getPlayer())) {
            ...
            final Type value2 = Type.get(item.getType());
            if (value2 != null) {
                if (!equippedItem.matches(value2, equipmentType)) {
                    continue;
                }
        ...
        for (final EquippedPlayerItem equippedPlayerItem : this.inventory.getEquipped()) {
            ...
            if (item2.hasData(ItemStats.ABILITIES) && (MMOItems.plugin.getConfig().getBoolean("abilities-bypass-encumbering", false) || !this.fullHands) && (equippedPlayerItem.getSlot() != Type.EquipmentSlot.OFF_HAND || !MMOItems.plugin.getConfig().getBoolean("disable-abilities-in-offhand"))) {
        ...
        while (iterator3.hasNext()) {
            final ItemSet value3 = MMOItems.plugin.getSets().get(iterator3.next().getItem().getNBT().getString("MMOITEMS_ITEM_SET"));
            if (value3 == null) {
                continue;
            }
            final int i = hashMap.getOrDefault(value3, 0) + 1;
    ...
    public ItemAttackResult castAbilities(final LivingEntity livingEntity, final ItemAttackResult itemAttackResult, final Ability.CastingMode castingMode) {
        if (!this.hasAbility(castingMode)) {
            return itemAttackResult;
        }
        return this.castAbilities(this.getStats().newTemporary(), livingEntity, itemAttackResult, castingMode);
    }
    ...
    @Deprecated
    public void cast(final Ability ability) {
        this.cast(this.getStats().newTemporary(), null, new ItemAttackResult(true, new DamageType[] { DamageType.SKILL }), new AbilityData(ability, Ability.CastingMode.RIGHT_CLICK));
    }
    
    public void cast(final AbilityData abilityData) {
        this.cast(this.getStats().newTemporary(), null, new ItemAttackResult(true, new DamageType[] { DamageType.SKILL }), abilityData);
    }
    ...
    public boolean canUseItem(final String s) {
        return (this.itemCooldowns.containsKey(s) ? this.itemCooldowns.get(s) : 0L) < System.currentTimeMillis();
    }
    ...
    public static PlayerData get(final OfflinePlayer offlinePlayer) {
        return get(offlinePlayer.getUniqueId());
    }
    
    public static PlayerData get(final UUID uuid) {
        return PlayerData.data.get(uuid);
    }
    
    public static void load(final Player player) {
        if (!PlayerData.data.containsKey(player.getUniqueId())) {
            PlayerData.data.put(player.getUniqueId(), new PlayerData(MMOPlayerData.get((OfflinePlayer)player)));
            return;
        }
        final PlayerData value = get(player.getUniqueId());
