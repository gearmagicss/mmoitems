...
public class ThunderSpirit implements StaffAttackHandler
{
    ...
    public void handle(final PlayerStats.CachedStats cachedStats, final NBTItem nbtItem, final double n, final double n2) {
        ...
        new BukkitRunnable() {
            final Location target = ThunderSpirit.this.getGround(cachedStats.getPlayer().getTargetBlock((Set)null, (int)n2 * 2).getLocation()).add(0.0, 1.2, 0.0);
            ...
            public void run() {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    this.loc.getWorld().spawnParticle(Particle.FIREWORKS_SPARK, this.loc, 0, 0.03, 0.0, 0.03, 0.0);
                    if (this.ti >= 1.0) {
