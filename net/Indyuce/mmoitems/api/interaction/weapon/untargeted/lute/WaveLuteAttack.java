        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    final Vector rotateFunc = MMOUtils.rotateFunc(new Vector(0.5, 0.0, 0.0), this.loc);
                    if (nbtItem.hasTag("MMOITEMS_PROJECTILE_PARTICLES")) {
                        ...
                        if (ProjectileParticlesData.isColorable(value)) {
                            ...
                            ProjectileParticlesData.shootParticle(cachedStats.getPlayer(), value, this.loc.clone().add(rotateFunc.multiply(Math.sin(this.ti / 2.0))), double1, double2, double3);
                            ...
                            ProjectileParticlesData.shootParticle(cachedStats.getPlayer(), value, this.loc.clone().add(rotateFunc.multiply(Math.sin(this.ti / 2.0))), 0.0, 0.0, 0.0);
                            ProjectileParticlesData.shootParticle(cachedStats.getPlayer(), value, this.loc.clone().add(rotateFunc.multiply(-1)), 0.0, 0.0, 0.0);
                        }
                    }
                    else {
                        this.loc.getWorld().spawnParticle(Particle.NOTE, this.loc.clone().add(rotateFunc.multiply(Math.sin(this.ti / 2.0))), 0);
