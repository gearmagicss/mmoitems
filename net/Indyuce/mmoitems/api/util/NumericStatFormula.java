...
public class NumericStatFormula implements RandomStatData, UpdatableRandomStatData
{
    ...
    public double calculate(final double n, final double n2) {
        ...
        final double n3 = this.base + this.scale * n;
        double maxSpread = n2 * this.spread;
        if (maxSpread < -this.getMaxSpread()) {
            maxSpread = -this.getMaxSpread();
        }
        else if (maxSpread > this.getMaxSpread()) {
            maxSpread = this.getMaxSpread();
        }
        return n3 + maxSpread;
    }
