...
public class ItemStackBuilder
{
    ...
    public NBTItem buildNBT(final boolean b) {
        ...
        for (final ItemStat itemStat : clone.getStats()) {
            try {
                ...
                if (statHistory != null) {
                    ...
                    if (!statHistory.isClear() || itemStat instanceof Enchants) {
        ...
        if (clone.hasData(ItemStats.LORE)) {
            ...
            ((StringListData)clone.getData(ItemStats.LORE)).getList().forEach(s -> list.add(this.lore.applyLorePlaceholders(s)));
        ...
        while (iterator3.hasNext()) {
            jsonArray.add((String)iterator3.next());
        }
        if (jsonArray.size() != 0) {
        ...
        if (this.mmoitem.hasData(ItemStats.NAME) && this.meta.hasDisplayName()) {
            this.meta.setDisplayName(this.getMeta().getDisplayName());
        }
    ...
    public ItemStack build() {
        return new DynamicLore(this.buildNBT()).build();
    }
    
    public ItemStack build(final boolean b) {
        return new DynamicLore(this.buildNBT(b)).build();
    }
