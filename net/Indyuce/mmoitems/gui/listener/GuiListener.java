...
public class GuiListener implements Listener
{
    ...
    public void a(final InventoryClickEvent inventoryClickEvent) {
        ...
        if (inventoryClickEvent.getInventory().getHolder() instanceof PluginInventory) {
            ...
            final MMOItemTemplate edited = ((EditionInventory)pluginInventory).getEdited();
            if (currentItem.getItemMeta().getDisplayName().equals(ChatColor.GREEN + "\u21e8" + " Back")) {
                ...
                else if (pluginInventory instanceof RecipeEdition) {
                    new RecipeListEdition(player, edited).open(((EditionInventory)pluginInventory).getPreviousPage());
                }
                else {
                    new ItemEdition(player, edited).onPage(((EditionInventory)pluginInventory).getPreviousPage()).open();
                }
            }
        }
    }
