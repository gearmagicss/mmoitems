...
public class CompatibleTypes extends ItemStat
{
    public CompatibleTypes() {
        super("COMPATIBLE_TYPES", VersionMaterial.COMMAND_BLOCK.toMaterial(), "Compatible Types", new String[] { "The item types this skin is", "compatible with." }, new String[] { "skin" }, new Material[0]);
    }
    ...
    public void whenClicked(@NotNull final EditionInventory editionInventory, @NotNull final InventoryClickEvent inventoryClickEvent) {
        if (inventoryClickEvent.getAction() == InventoryAction.PICKUP_ALL) {
            new StatEdition(editionInventory, ItemStats.COMPATIBLE_TYPES, new Object[0]).enable("Write in the chat the name of the type you want to add.");
        }
        if (inventoryClickEvent.getAction() == InventoryAction.PICKUP_HALF && editionInventory.getEditedSection().contains("compatible-types")) {
            final List stringList = editionInventory.getEditedSection().getStringList("compatible-types");
            if (stringList.size() < 1) {
                return;
            }
            final String str = stringList.get(stringList.size() - 1);
            stringList.remove(str);
            editionInventory.getEditedSection().set("compatible-types", (Object)stringList);
    ...
    public void whenInput(@NotNull final EditionInventory editionInventory, @NotNull final String s, final Object... array) {
        final List<String> list = editionInventory.getEditedSection().contains("compatible-types") ? editionInventory.getEditedSection().getStringList("compatible-types") : new ArrayList<String>();
        list.add(s.toUpperCase());
        editionInventory.getEditedSection().set("compatible-types", (Object)list);
        editionInventory.registerTemplateEdition();
        editionInventory.getPlayer().sendMessage(MMOItems.plugin.getPrefix() + "Compatible Types successfully added.");
    }
    ...
    public void whenDisplayed(final List<String> list, final Optional<RandomStatData> optional) {
        ...
        list.add("");
        list.add(ChatColor.YELLOW + "\u25ba" + " Click to add a new type.");
        list.add(ChatColor.YELLOW + "\u25ba" + " Right click to remove the last type.");
    }
    ...
    public void whenApplied(@NotNull final ItemStackBuilder itemStackBuilder, @NotNull final StatData statData) {
        itemStackBuilder.getLore().insert("compatible-types", new ArrayList<String>(((StringListData)statData).getList()));
