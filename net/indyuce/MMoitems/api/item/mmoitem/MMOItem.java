...
public class MMOItem implements ItemReference
{
    ...
    public int getDamage() {
        if (!this.hasData(ItemStats.ITEM_DAMAGE)) {
            return 0;
        }
        return SilentNumbers.round(((DoubleData)this.getData(ItemStats.ITEM_DAMAGE)).getValue());
    }
    
    public void setDamage(final int n) {
        if (this.hasData(ItemStats.UNBREAKABLE)) {
            return;
        }
        this.setData(ItemStats.ITEM_DAMAGE, new DoubleData(n));
    }
    ...
    public UpgradeTemplate getUpgradeTemplate() {
        Validate.isTrue(this.hasUpgradeTemplate(), "Item without upgrade information");
    ...
    public ArrayList<MMOItem> extractGemstones() {
        ...
        for (final GemstoneData gemstoneData : gemSocketsData.getGemstones()) {
            final MMOItem mmoItem = MMOItems.plugin.getMMOItem(MMOItems.plugin.getTypes().get(gemstoneData.getMMOItemType()), gemstoneData.getMMOItemID());
            if (mmoItem != null) {
