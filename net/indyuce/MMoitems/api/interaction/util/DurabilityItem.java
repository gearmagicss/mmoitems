...
public class DurabilityItem
{
    ...
    public DurabilityItem(@Nullable final Player player, @NotNull final ItemStack itemStack) {
        this(player, NBTItem.get(itemStack));
    }
    
    public DurabilityItem(@Nullable final Player player, @NotNull final NBTItem nbtItem) {
        this.player = player;
        Validate.notNull((Object)(this.nbtItem = nbtItem), "Item cannot be null");
        this.maxDurability = this.nbtItem.getInteger("MMOITEMS_MAX_DURABILITY");
        final int n = this.nbtItem.hasTag("MMOITEMS_DURABILITY") ? this.nbtItem.getInteger("MMOITEMS_DURABILITY") : this.maxDurability;
        this.durability = n;
        this.initialDurability = n;
    ...
    public DurabilityItem addDurability(final int n) {
        ...
        this.durability = Math.min(this.durability + n, this.maxDurability);
        return this;
    }
    ...
    public ItemStack toItem() {
        ...
        final ItemStack item = this.nbtItem.toItem();
        final String colors = MythicLib.inst().parseColors(MMOItems.plugin.getLanguage().getStatFormat("durability").replace("#m", "" + this.maxDurability));
        return new LoreUpdate(item, colors.replace("#c", "" + this.initialDurability), colors.replace("#c", "" + this.durability)).updateLore();
    }
