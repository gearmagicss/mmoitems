...
public class EnchantListData implements StatData, Mergeable
{
    ...
    public void identifyTrueOriginalEnchantments(@NotNull final MMOItem mmoItem) {
        ...
        if (enchantListData.getEnchants().size() == 0) {
        ...
        for (final Enchantment enchantment : this.getEnchants()) {
            ...
            final int level2 = enchantListData.getLevel(enchantment);
        ...
        if (!enchantListData2.isClear()) {
            mmoItem.mergeData(ItemStats.ENCHANTS, enchantListData2, null);
        }
    }
}
