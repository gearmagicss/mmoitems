...
public final class ScannerImpl implements Scanner
{
    ...
    private void fetchMoreTokens() {
        ...
        final String text = String.format("found character %s '%s' that cannot start any token. (Do not use %s for indentation)", ch, chRepresentation, chRepresentation);
    ...
    private String scanFlowScalarNonSpaces(final boolean doubleQuoted, final Mark startMark) {
        ...
        while (true) {
            ...
            if (!doubleQuoted && ch == '\'' && this.reader.peek(1) == '\'') {
                ...
                this.reader.forward();
                ch = this.reader.peek();
                if (ScannerImpl.ESCAPE_REPLACEMENTS.containsKey(ch)) {
                    chunks.append(ScannerImpl.ESCAPE_REPLACEMENTS.get(ch));
                    this.reader.forward();
                }
                else if (ScannerImpl.ESCAPE_CODES.containsKey(ch)) {
                    length = ScannerImpl.ESCAPE_CODES.get(ch);
                    this.reader.forward();
