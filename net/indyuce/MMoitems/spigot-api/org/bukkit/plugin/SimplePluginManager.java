...
public final class SimplePluginManager implements PluginManager
{
    ...
    public void registerInterface(final Class<? extends PluginLoader> loader) throws IllegalArgumentException {
        if (PluginLoader.class.isAssignableFrom(loader)) {
            ...
            synchronized (this) {
                Pattern[] array;
                for (int length = (array = patterns).length, i = 0; i < length; ++i) {
            ...
            if (loader != null) {
                ...
                try {
                    ...
                    if (description.rawName.indexOf(32) != -1) {
                        this.server.getLogger().warning(String.format("Plugin `%s' uses the space-character (0x20) in its name `%s' - this is discouraged", description.getFullName(), description.rawName));
                    }
                }
        ...
        while (!plugins.isEmpty()) {
            ...
            while (pluginIterator.hasNext()) {
                ...
                if (!dependencies.containsKey(plugin) && !softDependencies.containsKey(plugin) && plugins.containsKey(plugin)) {
                    ...
                    try {
                        ...
                        this.server.getLogger().log(Level.SEVERE, "Could not load '" + file3.getPath() + "' in folder '" + directory.getPath() + "'", ex2);
                    }
                }
            }
            if (missingDependency) {
                ...
                while (failedPluginIterator.hasNext()) {
                    final File file3 = failedPluginIterator.next();
                    failedPluginIterator.remove();
                    this.server.getLogger().log(Level.SEVERE, "Could not load '" + file3.getPath() + "' in folder '" + directory.getPath() + "': circular dependency detected");
                }
            }
        }
    ...
    private void fireEvent(final Event event) {
        ...
        RegisteredListener[] array;
        for (int length = (array = listeners).length, i = 0; i < length; ++i) {
            final RegisteredListener registration = array[i];
