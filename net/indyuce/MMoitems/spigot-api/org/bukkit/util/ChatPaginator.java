...
public class ChatPaginator
{
    ...
    public static String[] wordWrap(final String rawString, final int lineLength) {
        ...
        final char[] rawChars = (String.valueOf(rawString) + ' ').toCharArray();
        ...
        for (int i = 0; i < rawChars.length; ++i) {
            ...
            if (c == '§') {
                ...
                if (line.length() == 0 && word.length() > lineLength) {
                    String[] split;
                    for (int length = (split = word.toString().split("(?<=\\G.{" + lineLength + "})")).length, j = 0; j < length; ++j) {
                        final String partialWord = split[j];
                    ...
                    String[] split2;
                    for (int length2 = (split2 = word.toString().split("(?<=\\G.{" + lineLength + "})")).length, k = 0; k < length2; ++k) {
                        final String partialWord = split2[k];
                        lines.add(line.toString());
