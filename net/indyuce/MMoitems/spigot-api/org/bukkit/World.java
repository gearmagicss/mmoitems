...
public interface World extends PluginMessageRecipient, Metadatable
{
    ...
    Arrow spawnArrow(final Location p0, final Vector p1, final float p2, final float p3);
     ...
     <T extends Arrow> T spawnArrow(final Location p0, final Vector p1, final float p2, final float p3, final Class<T> p4);
    ...
    Collection<Entity> getNearbyEntities(final Location p0, final double p1, final double p2, final double p3);
    ...
    public enum Environment
    {
        ...
        static {
            ...
            for (int length = (values = values()).length, i = 0; i < length; ++i) {
                final Environment env = values[i];
                Environment.lookup.put(env.getId(), env);
            }
        }
        
        private Environment(final String name, final int ordinal, final int id) {
            this.id = id;
        }
