    ...
    public void blockDrops(final BlockBreakEvent blockBreakEvent) {
        ...
        if (fromBlock.isPresent()) {
            ...
            if (this.customBlocks.containsKey(customBlock.getId()) && CustomBlockListener.getPickaxePower(player) >= customBlock.getRequiredPower()) {
                ...
                final CustomBlock customBlock2;
                final PlayerData playerData;
                final List<ItemStack> list;
                final CustomBlockDropEvent customBlockDropEvent;
                final Iterator<ItemStack> iterator;
                final Block block2;
                Bukkit.getScheduler().runTaskLater((Plugin)MMOItems.plugin, () -> {
                    PlayerData.get((OfflinePlayer)player2);
                    this.customBlocks.get(customBlock2.getId()).read(playerData, this.hasSilkTouchTool(player2));
                    customBlockDropEvent = new CustomBlockDropEvent(playerData, customBlock2, list);
                    Bukkit.getPluginManager().callEvent((Event)customBlockDropEvent);
            ...
            Bukkit.getScheduler().runTaskLater((Plugin)MMOItems.plugin, () -> {
                block.getType();
                this.blocks.get(o).read(PlayerData.get((OfflinePlayer)player3), this.hasSilkTouchTool(player3)).iterator();
