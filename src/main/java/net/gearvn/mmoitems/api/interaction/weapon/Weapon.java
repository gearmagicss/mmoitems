...
public class Weapon extends UseItem
{
    ...
    public boolean applyItemCosts() {
        if (MMOUtils.twoHandedCase(this.getPlayer())) {
        ...
        boolean flagAllowed = true;
        final FlagPlugin flags = MMOItems.plugin.getFlags();
        if (flags != null) {
            flagAllowed = flags.isFlagAllowed(this.getPlayer(), FlagPlugin.CustomFlag.MI_WEAPONS);
        }
        else {
            MMOItems.print(Level.WARNING, "$fFlag plugin not found", null, new String[0]);
        }
        return canUse || flagAllowed;
    }
