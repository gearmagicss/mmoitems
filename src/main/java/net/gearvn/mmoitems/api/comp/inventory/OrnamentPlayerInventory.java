...
public class OrnamentPlayerInventory implements PlayerInventory, Listener
{
    ...
    public List<EquippedItem> getInventory(final Player player) {
        ...
        for (final ItemStack itemStack : player.getInventory().getContents()) {
            ...
            if (itemStack != null && (nbtItem = MythicLib.plugin.getVersion().getWrapper().getNBTItem(itemStack)).hasType() && Type.get(nbtItem.getType()).getEquipmentType() == Type.EquipmentSlot.ANY) {
                list.add(new EquippedItem(nbtItem, Type.EquipmentSlot.ANY));
            }
        }
        return list;
    }
    ...
    public void a(final EntityPickupItemEvent entityPickupItemEvent) {
        if (entityPickupItemEvent.getEntityType() == EntityType.PLAYER) {
            ...
            if (value.hasType() && Type.get(value.getType()).getEquipmentType() == Type.EquipmentSlot.ANY) {
